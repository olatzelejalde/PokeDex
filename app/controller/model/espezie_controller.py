class EspezieController:
    ...
    def get_type_effectiveness(self, pokemon_name):
        ...
        poke = dict(row[0])
        nire_motak = []
        if poke.get('mota1'): nire_motak.append(poke['mota1'])
        if poke.get('mota2'): nire_motak.append(poke['mota2'])

        # 2. Datu-baseko mota posible guztien zerrenda lortu
        rows_motak = self.db.select("SELECT izena FROM mota")
        mota_guztiak = [r['izena'] for r in rows_motak]
        ...
        for atacante in mota_guztiak:
            mult = 1.0
            for defensor in nire_motak:
                res = self.db.select("""
    ...
    def get_scan_info(self, izena):
        ...
        nire_motak = []
        if data.get('mota1'): nire_motak.append(data['mota1'])
        if data.get('mota2'): nire_motak.append(data['mota2'])
